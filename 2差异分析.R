...
macdeg <- FindMarkers(tdm,ident.1 = 'Macrophage_P-TDM',ident.2 = 'Macrophage_M-TDM', verbose = TRUE, test.use = 'wilcox', min.pct = 0.1)
head(macdeg)

neudeg <- FindMarkers(tdm,ident.1 = 'Neutrophil_P-TDM',ident.2 = 'Neutrophil_M-TDM', verbose = TRUE, test.use = 'wilcox', min.pct = 0.1)
head(neudeg)
...
mactop10 <- macdeg %>% top_n(n = 10, wt = avg_log2FC) %>% row.names()
mactop10

neutop10 <- neudeg %>% top_n(n = 10, wt = avg_log2FC) %>% row.names()
neutop10
...
EnhancedVolcano(macdeg, lab = rownames(macdeg), x = 'avg_log2FC', y = 'p_val_adj',
                pCutoff = 0.05, FCcutoff = 0.5, pointSize = 3.0, labSize = 6.0,
                title = 'diff_Macrophage')

EnhancedVolcano(neudeg, lab = rownames(neudeg), x = 'avg_log2FC', y = 'p_val_adj',
                pCutoff = 0.05, FCcutoff = 0.5, pointSize = 3.0, labSize = 6.0,
                title = 'diff_Neutrophil')

#手动读入
cd4 <- read_csv("./25年/差异基因/CD4+ T.CSV")
cd8 <- read_csv("./25年/差异基因/CD8+ T.CSV")
mac <- read_csv("./25年/差异基因/Macrophage.CSV")
b <- read_csv("./25年/差异基因/B.CSV")
cd4top15 <- cd4 %>% top_n(n = 15, wt = avg_log2FC) %>% pull(`...1`) #读入csv会多一列...1，里面存着基因
cd4top15

p <- EnhancedVolcano(cd4, lab = cd4$`...1`, x = 'avg_log2FC', y = 'p_val_adj',
                pCutoff = 0.05, FCcutoff = 1, pointSize = 3, labSize = 4.0,
                selectLab = cd4$`...1`[cd4$p_val_adj < 0.05 & abs(cd4$avg_log2FC) > 1],
                max.overlaps = Inf,
                ...
                title = 'Differential gene_CD4+ T', subtitle = NULL) + theme(legend.position = "none")

ggsave("output.pdf", plot = p, device = "pdf", width = 15, height = 7, dpi = 600, limitsize = FALSE)

p <- EnhancedVolcano(cd8, lab = cd8$`...1`, x = 'avg_log2FC', y = 'p_val_adj',
                pCutoff = 0.05, FCcutoff = 1, pointSize = 3, labSize = 4.0,
                selectLab = cd8$`...1`[cd8$p_val_adj < 0.05 & abs(cd8$avg_log2FC) > 1],
                max.overlaps = Inf,
                col = c("grey20","grey20","grey20","red"),
                labCol = 'black', labFace = 'bold', boxedLabels = TRUE, drawConnectors = TRUE,
                endsConnectors = "last", widthConnectors = 0.8,
                title = 'Differential gene_CD8+ T', subtitle = NULL) + theme(legend.position = "none")
p <- EnhancedVolcano(mac, lab = mac$`...1`, x = 'avg_log2FC', y = 'p_val_adj',
                pCutoff = 0.05, FCcutoff = 1, pointSize = 3, labSize = 4.0,
                selectLab = mac$`...1`[mac$p_val_adj < 0.05 & abs(mac$avg_log2FC) > 1],
                max.overlaps = Inf,
                col = c("grey20","grey20","grey20","red"),
                labCol = 'black', labFace = 'bold', boxedLabels = TRUE, drawConnectors = TRUE,
                endsConnectors = "last", widthConnectors = 0.8,
                title = 'Differential gene_Macrophage', subtitle = NULL) + theme(legend.position = "none")

p <- EnhancedVolcano(b, lab = b$`...1`, x = 'avg_log2FC', y = 'p_val_adj',
                pCutoff = 0.05, FCcutoff = 1, pointSize = 3, labSize = 4.0,
                selectLab = b$`...1`[b$p_val_adj < 0.05 & abs(b$avg_log2FC) > 1],
                max.overlaps = Inf,
                col = c("grey20","grey20","grey20","red"),
                labCol = 'black', labFace = 'bold', boxedLabels = TRUE, drawConnectors = TRUE,
                endsConnectors = "last", widthConnectors = 0.8,
                title = 'Differential gene_B', subtitle = NULL) + theme(legend.position = "none")

EnhancedVolcano(cd8, lab = cd8$`...1`, x = 'avg_log2FC', y = 'p_val_adj',
                pCutoff = 0.05, FCcutoff = 0.5, pointSize = 3.0, labSize = 6.0,
                selectLab = cd4$`...1`[cd4$p_val_adj < 0.05 & abs(cd4$avg_log2FC) > 0.5],
                title = 'diff_CD8+ T')

EnhancedVolcano(mac, lab = mac$`...1`, x = 'avg_log2FC', y = 'p_val_adj',
                pCutoff = 0.05, FCcutoff = 0.5, pointSize = 3.0, labSize = 6.0,
                selectLab = cd4$`...1`[cd4$p_val_adj < 0.05 & abs(cd4$avg_log2FC) > 0.5],
                title = 'diff_Macrophage')

EnhancedVolcano(neu, lab = neu$`...1`, x = 'avg_log2FC', y = 'p_val_adj',
                pCutoff = 0.05, FCcutoff = 0.5, pointSize = 3.0, labSize = 6.0,
                selectLab = cd4$`...1`[cd4$p_val_adj < 0.05 & abs(cd4$avg_log2FC) > 0.5],
                title = 'diff_Neutrophil')
